#!/usr/bin/env python3
"""
Fixed-gate synthetic evaluation for anomaly detection.
Calculates AUROC and point-adjusted F1 at the best threshold.
"""
...
# =============================================================================
# BOILERPLATE (Argument Parsing & Loading)
# =============================================================================
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Fixed-gate synthetic evaluation")
    ...
    # Accepted by the CLI for completeness, but not used in the current
...
# =============================================================================
# MODEL LOADING (Copied from your script)
# =============================================================================
...
def get_model_and_score_fns(args, model_name, feature_cols, device):
    if model_name == "gated":
        ...
        def eval_batch(x, y):
            out = model(x, y)
            if args.gated_score == "model_score":
                alpha = out["forecast_loss"].new_full(out["forecast_loss"].shape, args.fixed_gate_alpha)
...
def run_evaluation(args):
    ...
    for alpha in alphas:
        args.fixed_gate_alpha = alpha
        for model_name in models:
            logging.info(f"--- Evaluating Model: {model_name} | fixed_gate_alpha={alpha} ---")
            ...
            for path in test_files:
                ...
                print(
                    f"File: {path.name:<20} | alpha={alpha:.3f} | "
                    f"AUROC: {metrics['auroc']:.4f} | PA-F1: {metrics['adj_f1']:.4f}"
                )
    ...
    # Aggregate per alpha across files
    agg_rows = []
    for (model_name, alpha), group in out_df.groupby(["model", "fixed_gate_alpha"]):
        sum_tp = int(group["tp"].sum())
        sum_fp = int(group["fp"].sum())
        sum_fn = int(group["fn"].sum())
        sum_tn = int(group["tn"].sum())
        agg_prec = sum_tp / (sum_tp + sum_fp + 1e-10)
        agg_rec = sum_tp / (sum_tp + sum_fn + 1e-10)
        agg_f1 = 2 * (agg_prec * agg_rec) / (agg_prec + agg_rec + 1e-10)
        agg_rows.append({
            "model": model_name,
            "fixed_gate_alpha": alpha,
            "mean_auroc": float(group["auroc"].mean()),
            "mean_best_threshold": float(group["best_threshold"].mean()),
            "mean_adj_f1": float(group["adj_f1"].mean()),
            "mean_adj_precision": float(group["adj_precision"].mean()),
            "mean_adj_recall": float(group["adj_recall"].mean()),
            "sum_tp": sum_tp,
            "sum_fp": sum_fp,
            "sum_fn": sum_fn,
            "sum_tn": sum_tn,
            "agg_adj_precision": float(agg_prec),
            "agg_adj_recall": float(agg_rec),
            "agg_adj_f1": float(agg_f1),
        })

    agg_df = pd.DataFrame(agg_rows)
    agg_path = args.outdir / "event_evaluation_aggregate.csv"
    agg_df.to_csv(agg_path, index=False)
    
    print("\n" + "="*60)
    print("FINAL SUMMARY (Mean across files)")
    print("="*60)
    summary = out_df.groupby(["model", "fixed_gate_alpha"])[["auroc", "adj_f1"]].mean()
    print(summary)
