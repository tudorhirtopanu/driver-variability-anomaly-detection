...
python run_experiments.py --model lstm_forecast
...
# Make sure repository root is on sys.path when running this file directly.
...
def aggregate_in_memory(
    ...
    agg = {
        ...
        # Feature-wise FP MSE averages (useful for bar plots)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=MODEL_TYPES,
        required=True,
        help="Which model to run: lstm, transformer, transformer_forecast, lstm_forecast, lstm_vae, pca, tcn, persistence, usad",
    )
    args = parser.parse_args()
    model_type_str = args.model.lower()

    set_global_seeds(RANDOM_SEED)

    from fp_analysis.sonata.evaluate import run_single_split
    ...
    if not splits_path.exists():
        raise FileNotFoundError(
            f"Splits config not found at {splits_path}. Run scripts/generate_fp_splits.sh sonata first."
        )

    total_drivers = len(ALL_DRIVERS)

    for num_seen in NUM_SEEN_OPTIONS:
        print(f"\n=== {model_type_str.upper()} | {num_seen}/{total_drivers - num_seen} splits "
