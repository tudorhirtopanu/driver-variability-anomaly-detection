"""Unified evaluation entrypoint for the Sonata false-positive study."""
...
from fp_analysis.sonata.data_utils import prepare_split
from fp_analysis.sonata.model_config import MODEL_CONFIG
...
FORECAST_MODELS = {"transformer_forecast", "lstm_forecast"}
...
def _collect_reconstruction_scores(model: torch.nn.Module, dataset) -> np.ndarray:
    model.eval()
...
def _collect_reconstruction_feature_mse(model: torch.nn.Module, dataset) -> np.ndarray:
    model.eval()
...
def _collect_forecast_scores(model: torch.nn.Module, dataset) -> np.ndarray:
    model.eval()
...
def _collect_forecast_feature_mse(model: torch.nn.Module, dataset) -> np.ndarray:
    model.eval()
...
def _false_positive_rate(scores: np.ndarray, tau: float) -> float:
    if scores.size == 0:
        return 0.0
...
def _run_split_evaluation(
    *,
    ...
    split = prepare_split(train_drivers=train_drivers, unseen_drivers=unseen_drivers)
    ...
    fpr_seen = _false_positive_rate(scores_seen_test, tau)
    fpr_unseen = _false_positive_rate(scores_unseen_test, tau)
    ...
    seen_fprs = _fpr_per_driver(scores_seen_test, seen_test_driver_ids_win, tau)
    for driver, value in sorted(seen_fprs.items()):
        print(f"  Seen   driver {driver}: {value:.4f}")
    unseen_fprs = _fpr_per_driver(scores_unseen_test, unseen_test_driver_ids_win, tau)
    ...
    summary = _summarize_fp_analysis(
...
def run_single_split(
    ...
    if model_key in FORECAST_MODELS:
        return _run_split_evaluation(
            ...
            score_fn=_collect_forecast_scores,
            feature_mse_fn=_collect_forecast_feature_mse,
            label_prefix="[Transformer-Forecast]",
    ...
    return _run_split_evaluation(
        ...
        score_fn=_collect_reconstruction_scores,
        feature_mse_fn=_collect_reconstruction_feature_mse,
        label_prefix=label_prefix,
